...
'''+json.dumps({'news':self.m.news,'shop':self.payload.get('weaponShopList',[]),
    ...
    def acquire(self):
        ...
        for point in self.points():
            ...
            if allowed and allowed < MIN_TASK_ROUNDS + routes.distance(target):
                ...
                self.event('TASK_OFFER_SKIP', left=allowed,
                           need=MIN_TASK_ROUNDS + routes.distance(target))
                continue
    ...
    def solve(self):
        ...
        if not any(distance(role.pos,q)<=1 for q in task['positions']):
            ...
            # 否则"接了任务却再也不去"会一直拖到超时(夜里战斗未结束时先守阵位)。
...
- 中文参数值必须百分号编码：用 curl -s -G "<url>" --data-urlencode "city=北京"（直接把中文写进 URL，服务端会按 latin-1 解出乱码，实测白花 2 回合）。
- 一次调用要试的参数组合写在同一条命令里（例如同时试 location/city 两种参数名、Authorization/X-API-Key 两种鉴权头），不要用多个回合逐次试错。
- 修复类任务：进检查脚本前先去 Windows 回车（sed -i.bak 's/\r$//' check），并把"建目录/改第N行/加执行权限/跑检查"一次性串进同一条命令。
