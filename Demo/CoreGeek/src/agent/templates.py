...
def city_name(text):
    ...
    if match and not any(c in match.group(1) for c in '<>{}'):
        # 占位符(如 ?city=<城市名>)不是城市名, 回落到题干里的"查询XX…"
...
CITY_PARAM_RE = re.compile(r'city|location|name|area|region|城市|地名', re.I)


def doc_query_command(desc, dump=''):
    """题干直接给了接口文档(GET <url>?<param>=<取值/占位符>)时, 按文档原样调一次。

    只填文档里出现的参数名, 取值只来自题干里的城市名 —— 不臆造接口与参数。
    这类任务(如天气查询)没有显式"答案格式"标签, 因此答案仍交回 LLM 组织;
    本模板只负责把"该调哪个接口、参数怎么编码"一次做对。
    """
    text = '%s\n%s' % (desc or '', dump or '')
    ...
    for raw in URL_RE.findall(text):
        ...
        parts = []
        for name in PARAM_RE.findall('?%s' % query):
            if name not in parts:
                parts.append(name)
        if not parts:
            continue
        filled = [f'{name}={city}' for name in parts if city and CITY_PARAM_RE.search(name)]
        if not filled or any(c in v for c in '<>{}' for v in filled):
            continue
        # 中文取值必须百分号编码: 直接把中文写进 URL, Python/Flask 服务端按 latin-1
        # 解请求行会收到乱码(实测白花 2 个回合)。
...
def plan(desc, dump=''):
    ...
    query = doc_query_command(desc, dump)
    if query:
